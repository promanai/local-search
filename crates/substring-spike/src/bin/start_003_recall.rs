use localsearch_substring_spike::{
    ExperimentError, RecallBenchmarkOptions, Strategy, run_recall_benchmark,
};
use std::env;
use std::path::PathBuf;

fn main() -> Result<(), ExperimentError> {
    let mut options = RecallBenchmarkOptions {
        run_id: String::new(),
        records: 100_000,
        seed: 20_260_814,
        samples_per_case: 30,
        ...
        candidate_limit: 300,
        pressure_false_candidates: 350,
        baseline_index_bytes: 0,
        output_directory: PathBuf::from("reports/spikes/start-003-r"),
        strategies: Vec::new(),
        memory_bytes: 0,
        storage: String::new(),
        power: String::new(),
    ...
    while index < arguments.len() {
        ...
        match flag.as_str() {
            ...
            "--samples" => options.samples_per_case = parse(value)?,
            "--writer-heap-bytes" => options.writer_heap_bytes = parse(value)?,
            "--candidate-limit" => options.candidate_limit = parse(value)?,
            "--pressure-false-candidates" => options.pressure_false_candidates = parse(value)?,
            "--baseline-index-bytes" => options.baseline_index_bytes = parse(value)?,
    ...
    for report in run_recall_benchmark(&options)? {
